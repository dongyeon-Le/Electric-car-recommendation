...
function displaySpecs(specs, side) {
    ...
    const subsidyTitle = document.getElementById('subsidy-title');
    const subsidyRegion = document.getElementById('subsidy-region');
    const subsidySection = document.getElementById('subsidy-section');

    if (!subsidyId) {
        // 보조금 ID가 없을 경우, 해당 side의 <td> 요소들을 숨김
        if (side === 'left') {
            subsidyRegion.cells[1].innerHTML = '';
            subsidySection.cells[1].innerHTML = '';
        } else if (side === 'right') {
            subsidyRegion.cells[2].innerHTML = '';
            subsidySection.cells[2].innerHTML = '';
        }

        // 만약 두 개의 보조금 중 하나라도 없으면 보조금 타이틀 숨김
        if (elements.left.subsidyId === null && elements.right.subsidyId === null) {
            subsidyTitle.style.display = 'none';
        }
    } else {
        // 보조금 ID가 있을 경우 해당 섹션 표시
        subsidyTitle.style.display = '';
        subsidyRegion.style.display = '';
...
async function fetchSubsidyInfo(side) {
    ...
    try {
        ...
        if (response.ok) {
            document.getElementById(`${side}-national-subsidy`).innerText = `${data.국가보조금.toLocaleString()} 원`;
            document.getElementById(`${side}-local-subsidy`).innerText = `${data.지자체보조금.toLocaleString()} 원`;
        } else {
